...
export default function RejectedLeaves() {
  ...
  useEffect(() => {
    ...
    const fetchRejectedLeaves = async () => {
      setStatus("loading");
      try {
        const { data } = await api.post("/superadmin/rejected-leaves", {
          employeeId: empId,
        });
        ...
        if (data?.success) {
          setLeaves(data.rejectedLeaves || []);
          setTotal(data.totalRejectedLeaves || 0);
          setStatus("success");
        } else {
          setErrorMsg(data?.message || "Could not load rejected leaves.");
          setStatus("error");
        }
      } catch (err) {
        if (ignore) return;
        setErrorMsg(
          err?.response?.data?.message ||
            "Something went wrong while fetching rejected leaves."
        );
        setStatus("error");
      }
    };

    if (empId) fetchRejectedLeaves();
  ...
  return (
    <div className="min-h-screen w-full bg-[#05040c] text-slate-100 relative overflow-x-hidden">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10">
        <div className="absolute -top-40 -left-40 h-[28rem] w-[28rem] rounded-full bg-red-600/20 blur-[120px]" />
        <div className="absolute top-1/3 -right-40 h-[26rem] w-[26rem] rounded-full bg-indigo-600/20 blur-[120px]" />
        <div className="absolute bottom-0 left-1/4 h-[22rem] w-[22rem] rounded-full bg-rose-600/10 blur-[110px]" />
      </div>
...
<h1 className="mt-1 pb-1.5 text-3xl sm:text-4xl md:text-5xl font-black leading-[1.15] tracking-tight !bg-gradient-to-r !from-[#ff0055] !via-[#ff0000] !to-[#ff0099] bg-clip-text !text-transparent">
              ...
              <p className="mt-1.5 text-sm text-slate-400">
                Requests that didn't get approved, along with the reason.
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2 rounded-2xl border border-red-400/20 bg-red-400/5 px-4 py-3">
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 rounded-full border border-red-400/20 bg-red-400/10 px-3 py-1">
      ...
      {/* admin remark = rejection reason, emphasized */}
      {leave.adminRemark && (
        <div className="relative mt-3 flex gap-2 rounded-xl border border-red-400/20 bg-red-400/[0.06] p-3 text-sm text-red-100/85">
          <AlertOctagon className="mt-0.5 h-4 w-4 shrink-0 text-red-400" />
          <p className="line-clamp-3">{leave.adminRemark}</p>
        </div>
      )}

      {/* footer: rejected date */}
      <div className="relative mt-4 flex items-center justify-between border-t border-white/5 pt-3">
        <span className="text-[11px] text-slate-500">Rejected on</span>
...
function LoadingState() {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-24 text-slate-400">
      ...
      <p className="text-sm">Fetching rejected leave records…</p>
    </div>
  );
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/[0.02] py-24 text-center">
      ...
      <p className="text-sm font-medium text-slate-300">No rejected leaves</p>
      <p className="max-w-xs text-xs text-slate-500">
        This employee has a clean record — nothing's been turned down so far.
      </p>
    </div>
  );
}