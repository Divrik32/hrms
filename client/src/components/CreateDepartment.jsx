...
const CreateDepartment = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Newly created department fetch kore list update
...
const handleDeleteDepartment = async (id) => {
  ...
  const confirmed = window.confirm(
    `Are you sure you want to delete "${department?.departmentName || "this"}" department?`
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 p-4 sm:p-6 font-sans relative overflow-hidden">
      {/* Ambient glow */}
      ...
      <div className="relative w-full max-w-4xl mx-auto z-10">
        ...
        <motion.div
          ...
          className="relative bg-slate-900/80 backdrop-blur-xl border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/40 overflow-hidden"
        >
          {/* Top accent bar */}
          ...
          {/* Divider */}
          <div className="h-px mx-6 sm:mx-8 bg-white/[0.06]" />
          ...
          >
            ...
            >
              ...
              <div
                className={`flex items-center gap-3 bg-slate-800/60 border rounded-xl px-4 py-3 transition-all duration-200 ${
                  focusedField === "departmentName"
              ...
              >
                ...
                <input
                  ...
                  onFocus={() =>
                    setFocusedField("departmentName")
                  }
            ...
            >
              <label className="block text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">
                Description
                <span className="ml-2 normal-case text-slate-600 tracking-normal font-normal">
                  (optional)
                </span>
              </label>

              <div
                className={`flex gap-3 bg-slate-800/60 border rounded-xl px-4 py-3 transition-all duration-200 ${
                  focusedField === "description"
                    ? "border-indigo-500/60 ring-1 ring-indigo-500/20 bg-slate-800/80"
                    : "border-white/[0.08] hover:border-white/[0.14]"
                }`}
            ...
            <motion.button
              ...
              whileHover={{
                scale: creatingDepartment ? 1 : 1.01,
              }}
              whileTap={{
                scale: creatingDepartment ? 1 : 0.98,
              }}
        ...
        <motion.div
          ...
          className="mt-6 bg-slate-800 border border-slate-600 rounded-2xl shadow-xl shadow-black/30 p-5 sm:p-6"
        >
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-sm text-slate-300 mt-1">
                {safeDepartments.length} department
            ...
            <motion.button
              ...
              whileHover={{
                scale: fetchingDepartments ? 1 : 1.05,
              }}
              whileTap={{
                scale: fetchingDepartments ? 1 : 0.95,
              }}
              className="p-2.5 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-800/60 hover:!text-white transition disabled:opacity-50"
                ...
                {safeDepartments.map(
                  (department, index) => (
                    <motion.div
                      ...
                      className="p-4 rounded-xl border border-slate-600 bg-slate-900 hover:border-indigo-600/50 transition"
                    >
<motion.div
  whileHover={{ y: -3 }}
  transition={{ duration: 0.2 }}
  className="relative flex flex-col gap-2 p-4 rounded-xl border border-slate-600 bg-slate-800 hover:border-indigo-500/60 hover:shadow-lg hover:shadow-indigo-950/30 transition-colors duration-300"
>
  {/* Top row: icon + name + delete */}
  <div className="flex items-center justify-between gap-2">
    <div className="flex items-center gap-3 min-w-0">
      <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-indigo-500/25 to-violet-500/25 flex items-center justify-center shrink-0 ring-1 ring-indigo-500/30">
  ...
  {/* Details, aligned under the icon */}
  <div className="pl-12">
    <p className="text-xs text-indigo-300 truncate flex items-center gap-1">
      <span className="w-1 h-1 rounded-full bg-indigo-400 shrink-0" />
      {department.companyId?.companyName || "Company not found"}
    </p>
